...
db.collection("users").where('emailid',"==",userid).get().then((snapshot)=>{
    ...
    updatebookstatus = ()=>{
        db.collection("alldonations").add({bookname:this.state.bookname,requestid:this.state.requestid,requestedby:this.state.recievername,donorid:this.state.userid,requeststatus:"donorintrested"})
    }
    componentDidMount(){
